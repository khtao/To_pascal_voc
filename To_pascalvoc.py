...
import shutil
import numpy as np
from histech_dataset import HistechDataset
from tqdm import tqdm


def generate_xml(name, bbox, label, img_size=(370, 1224, 3),
    ...
    append_xml_node_attr('folder', parent=annotation, text='pathology')
    append_xml_node_attr('filename', parent=annotation, text=img_name)
    source = append_xml_node_attr('source', parent=annotation)
    append_xml_node_attr('database', parent=source, text='pathology')
    append_xml_node_attr('annotation', parent=source, text='pathology')
    append_xml_node_attr('image', parent=source, text='pathology')
    append_xml_node_attr('flickrid', parent=source, text='000000')
    owner = append_xml_node_attr('owner', parent=annotation)
    append_xml_node_attr('url', parent=owner, text='http://media/khtao/data/DataSets/pathology_data_2')
    ...
    for i in range(len(bbox)):
        label_i = label[i]
        cls = class_sets[label_i] if label_i < len(class_sets) else 'dontcare'
        cls = cls.lower()
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Convert DataSet into Pascal voc format')
    parser.add_argument('--DataSet_path', dest='DataSet_path',
                        help='path to DataSet root',
                        default='/media/khtao/data/DataSets/pathology_data_2', type=str)
    parser.add_argument('--out', dest='outdir',
                        help='path to voc-pathology',
                        default='/media/khtao/data/DataSets/pathologyVOC', type=str)
...
if __name__ == '__main__':
    ...
    _data_dir = args.DataSet_path
    _out_dir = args.outdir
    _draw = bool(args.draw)
    _dest_label_dir, _dest_img_dir, _dest_set_dir = build_voc_dirs(_out_dir)
    ...
    for dset in modeset:
        ...
        """
        data_set = HistechDataset(root=_data_dir, mode=dset)
        class_sets = data_set.label_names
        ...
        for i, (img, bbox, label) in tqdm(enumerate(data_set)):
            img_size = img.shape
            # rgb to bgr
            img = np.array(img[:, :, ::-1])
            stem = dset + str(i)
            doc, objs = generate_xml(stem, bbox, label, img_size, class_sets=class_sets, doncateothers=_doncateothers)
            if _draw:
